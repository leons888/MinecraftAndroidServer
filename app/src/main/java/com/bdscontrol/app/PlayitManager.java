...
final class PlayitManager {
 ...
 void start(String secret){new Thread(()->{try{if(!runtime.box64Present())throw new IllegalStateException(runtime.reason());if(!binary().isFile())throw new IllegalStateException("playit-cli is not installed yet");List<String> cmd=new ArrayList<>();cmd.add(runtime.box64().getAbsolutePath());cmd.add(binary().getAbsolutePath());if(secret!=null&&!secret.isEmpty()){cmd.add("--secret");cmd.add(secret);log.event("info","Starting Playit with supplied secret key");}else{log.event("info","Starting Playit without secret key; complete the interactive claim flow if the CLI asks for it");}pm.start(cmd,dir,runtime.env(dir),(src,line)->{parse(line);log.line("playit",line);});state="running";log.event("info","Playit tunnel agent started; the dashboard address is usable only while this agent is online");}catch(Exception e){state="error";log.event("error","Playit failed: "+e.getMessage());}},"playit-start").start();}
