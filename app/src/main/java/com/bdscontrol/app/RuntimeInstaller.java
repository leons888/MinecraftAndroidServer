...
final class RuntimeInstaller {
 ...
 void install(List<RuntimeManifest.Artifact> artifacts){new Thread(()->{try{
  for(RuntimeManifest.Artifact a:artifacts){
   ...
   events.message("info","Downloading "+a.id+" from "+a.url);
   File tmp=new File(root,"downloads/"+a.id);
   try {
    VerifiedDownloader.fetch(a.url,a.sha256,tmp,(d,t)->events.progress(a.id,d,t));
   } catch(Exception e) {
    throw new IOException("Could not download or verify "+a.id+". Check internet access, free storage, and retry Runtime install. URL: "+a.url, e);
   }
   ...
   else{File out=new File(root,"bin/"+a.id);out.getParentFile().mkdirs();out.delete();if(!tmp.renameTo(out))throw new IOException("cannot install "+a.id+" into app data");}
   tmp.delete();
